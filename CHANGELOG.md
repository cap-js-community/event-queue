...
- Distributed locks carry an ownership token: renew and release only affect the lock of the processing run which acquired it. A run which lost its lock stops processing further chunks.
...
- [Admin Service] Remove impl annotation
- [Namespaces] Remove old events only in the processing namespaces
- [Processing] Fix select query that could fail for old HANA DB-Adapter
