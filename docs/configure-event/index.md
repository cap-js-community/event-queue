...
| appNames                      | If this parameter is defined, the event is only processed on the specified application names. The application name is extracted from the environment variable VCAP_APPLICATION. If no value is defined, the event is processed on all connected applications. | null          |
