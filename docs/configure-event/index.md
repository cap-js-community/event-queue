...
| randomOffset                  | Overrides the global `randomOffsetPeriodicEvents`. Adds a randomized offset (in seconds) to the calculated next start time of a periodic event. This helps to stagger event execution and reduce load spikes on the application server, especially when many tenants use the same schedule. The random value is between 0 and the specified number of seconds.                                                                            | 0             |
| utc                           | Specifies whether the cron expression should be interpreted in UTC time. If set to true, the schedule will follow UTC; otherwise, it will use the server's local time zone.                                                                                                                                                                                                                                                               | false         |
